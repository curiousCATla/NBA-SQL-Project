"""
analysis15.py — Injury/Availability Tax: how much do teams pay for players who don't play?

Two SQL queries:
  1. per_player  — one row per (player, season) with salary wasted on missed games.
  2. per_season  — league-wide availability rate and player count per season.

Three visualisations (one figure, three panels):
  Panel 1 (top-left)  : Bar chart — top 10 player-seasons by salary wasted.
  Panel 2 (top-right) : Scatter plot — availability vs salary for those same 10 players.
  Panel 3 (bottom)    : Dual-axis line chart — avg availability rate (left axis)
                        and number of qualifying players (right axis) by season.

Output: images/availability_tax.png
"""
...
# ── Query 1: per-player availability tax ─────────────────────────────────────
#
...
per_player AS (
    SELECT
        ...
        -- Salary attributed to missed games:
...
SELECT Player, Season, ROUND(G,1) AS G, availability_rate, salary_millions,
       cost_per_game_m, salary_wasted_m
FROM per_player
ORDER BY salary_wasted_m DESC;
""", conn)

# ── Query 2: league-wide availability by season ───────────────────────────────
...
-- Rank each player's salary within their season so the filter is
-- relative (above median) rather than a fixed dollar threshold that
-- disadvantages players from lower-cap eras.
salary_pct AS (
    SELECT
        playerName,
        seasonStartYear,
...
SELECT
    ...
    ROUND(AVG(cs.G * 1.0 / 82), 3) AS avg_availability
FROM clean_stats cs
JOIN salary_pct s
    ON  cs.Player = s.playerName
    AND cs.Season = s.seasonStartYear
...
ax_bar     = fig.add_subplot(2, 2, 1)          # top-left
ax_scatter = fig.add_subplot(2, 2, 2)          # top-right
ax_line    = fig.add_subplot(2, 1, 2)          # full-width bottom

# ── Panel 1: Bar chart — top 10 salary wasted ────────────────────────────────

bars = ax_bar.barh(top10['label'], top10['salary_wasted_m'],
                   color='#e05c5c', edgecolor='white')
ax_bar.invert_yaxis()   # highest value at the top
ax_bar.set_xlabel('Salary Wasted ($M)', fontsize=10)
ax_bar.set_title('Top 10 Player-Seasons: Salary Paid for Missed Games', fontsize=11)

for bar, (_, row) in zip(bars, top10.iterrows()):
    ax_bar.text(
        bar.get_width() + 0.2,
        bar.get_y() + bar.get_height() / 2,
        f"${row['salary_wasted_m']:.1f}M  ({row['G']}Games)",
        va='center', fontsize=8
    )

ax_bar.set_xlim(0, top10['salary_wasted_m'].max() + 5)

# ── Panel 2: Scatter — availability vs salary for the same 10 players ────────

scatter = ax_scatter.scatter(
    top10['availability_rate'],
    top10['salary_millions'],
    s=top10['salary_wasted_m'] * 20,   # bubble size proportional to wasted salary
...
cbar.set_label('Salary Wasted ($M)', fontsize=9)
...
ax_scatter.set_xlabel('Availability Rate (G / 82)', fontsize=10)
ax_scatter.set_ylabel('Salary ($M)', fontsize=10)
ax_scatter.set_title('Top 10: Availability Rate vs Salary\n(bubble size = salary wasted)', fontsize=11)
...
ax_count = ax_line.twinx()   # second y-axis sharing the same x-axis
...
plt.suptitle('NBA Injury / Availability Tax (1990–2021)', fontsize=14, fontweight='bold', y=1.01)
plt.tight_layout()
os.makedirs('images', exist_ok=True)
plt.savefig('images/availability_tax.png', dpi=150, bbox_inches='tight')
plt.show()
