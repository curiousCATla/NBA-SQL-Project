...
salaries        = pd.read_csv(os.path.join('data', 'NBA_Salaries.csv'))
payroll         = pd.read_csv(os.path.join('data', 'NBA_Payroll.csv'))
...
for df in [player_stats, player_boxscores, salaries, payroll]:
    unnamed_cols = [col for col in df.columns if col.startswith('Unnamed')]
    df.drop(columns=unnamed_cols, inplace=True)

# 3. Sanity check — confirm shape and column names for each table
for name, df in [('player_stats', player_stats), ('player_boxscores', player_boxscores),
                 ('salaries', salaries), ('payroll', payroll)]:
    print(f"\n--- {name} ---")
    print(f"  Shape : {df.shape}")
    print(f"  Columns: {list(df.columns)}")

# 4. Create (or connect to) a SQLite database file.
#    If nba.db doesn't exist yet, SQLite creates it automatically.
...
#    so data stays in sync with the CSVs.
...
# 6. Quick verification — ask SQLite which tables now exist in the database
cursor = conn.cursor()
